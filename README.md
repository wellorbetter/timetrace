...
- **AI Recap** — 先由本地事实规则生成回顾，可选使用 OpenAI-compatible 模型增强；事实指标不可被模型改写
- **Nowline** — 把实时活动合并成类似歌词的语义流水，支持透明置顶、历史淡出和点击穿透，全程本地且不消耗 AI token
...
活动数据只保存在本地 SQLite（`%APPDATA%\TimeTrace\time.db`），Nowline 不发起网络请求。只有用户显式配置并启用 AI Recap 时，才会把界面展示的结构化事实快照发送到所选模型端点；日记正文仍需单独授权。

Nowline 的架构与隐私边界见 [docs/NOWLINE.md](docs/NOWLINE.md)。
