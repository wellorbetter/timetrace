...
选中日期后，右侧轮播会同步切换应用柱状图（页首主图）、使用占比、当日汇总、专注与提醒、应用明细、24 小时时段分布和使用历史；不需要在多个页面之间来回跳转。每个视图都可以在设置中隐藏或排序。
...
番茄钟支持专注、短休息、长休息、暂停、继续、跳过和停止；应用提醒只累计同一可执行程序持续位于前台的有效时间。切换应用、空闲、锁屏、睡眠或暂停追踪都会结束连续使用段，恢复后不会补算离开时间。

![番茄钟与应用连续使用提醒](docs/screenshots/focus-reminders.png)
...
3. 回到概览，在右侧轮播的 **专注提醒** 视图中使用“专注与提醒”卡片开始、暂停、继续、跳过、停止或重置番茄钟；主窗口隐藏后，也可以从系统托盘查看倒计时并执行常用操作。
...
应用规则只保存在本地 SQLite，以规范化可执行文件路径作为稳定匹配身份。提醒卡片、规则列表和系统通知都不会显示规则路径或窗口标题：超时通知只包含应用显示名和取整后的连续使用分钟数。正常的一秒计时不会写数据库，重启后番茄钟与当前连续使用段都会安全回到未运行状态。
