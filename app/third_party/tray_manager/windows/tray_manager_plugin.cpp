...
// GDI+ system headers emit C4458 under /WX; disable around the include only.
#pragma warning(push)
...
#pragma comment(lib, "msimg32")  // AlphaBlend for owner-drawn menu icons
...
class TrayManagerPlugin : public flutter::Plugin {
 ...
 private:
  ...
  // Owner-drawn menu item visuals (icon bitmap + label), keyed by item id.
  struct MenuItemVisual {
    std::wstring label;
    HBITMAP bitmap = nullptr;
  };
  std::map<int, MenuItemVisual> menu_visuals_;
  ULONG_PTR gdiplus_token_ = 0;

  static std::vector<unsigned char> DecodeBase64(const std::string& in);
  static HBITMAP LoadPngBitmap(const std::vector<unsigned char>& bytes);
  static HBITMAP LoadIconBitmap(const std::string& base64);
...
void TrayManagerPlugin::_CreateMenu(HMENU menu, flutter::EncodableMap args) {
  ...
  // Rebuild: drop visuals from the previous menu.
  for (auto& kv : menu_visuals_) {
    if (kv.second.bitmap) DeleteObject(kv.second.bitmap);
  }
  menu_visuals_.clear();
  ...
  for (flutter::EncodableValue item_value : items) {
    ...
    std::string iconB64;
    if (auto* icon = std::get_if<std::string>(ValueOrNull(item_map, "icon"))) {
      iconB64 = *icon;
    }
    ...
    // Owner-draw items that carry an icon so Windows shows a glyph next to
    // the label; text/background are painted in WM_DRAWITEM.
    HBITMAP iconBmp = nullptr;
    if (!iconB64.empty() && type.compare("normal") == 0) {
      iconBmp = LoadIconBitmap(iconB64);
      if (iconBmp) {
        uFlags |= MF_OWNERDRAW;
      }
    }
    ...
    if (type.compare("separator") == 0) {
      ...
      if (uFlags & MF_OWNERDRAW) {
        MenuItemVisual v;
        v.label = g_converter.from_bytes(label);
        v.bitmap = iconBmp;
        menu_visuals_[id] = v;
        AppendMenuW(menu, uFlags, item_id, nullptr);
      } else {
...
std::vector<unsigned char> TrayManagerPlugin::DecodeBase64(
    const std::string& in) {
  static const char* tbl =
      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
  int val[256];
  for (int i = 0; i < 256; i++) val[i] = -1;
  for (int i = 0; i < 64; i++) val[(unsigned char)tbl[i]] = i;
...
HBITMAP TrayManagerPlugin::LoadPngBitmap(
  ...
  // Copy pixels into a top-down 32bpp DIB with premultiplied alpha so
  // AlphaBlend paints it correctly in WM_DRAWITEM.
  ...
  if (hbmp && bits) {
    ...
    if (bmp->LockBits(&rect, Gdiplus::ImageLockModeRead,
      ...
      for (int y = 0; y < h; y++) {
        unsigned char* src =
            (unsigned char*)bd.Scan0 + (size_t)y * bd.Stride;
        unsigned char* d = dst + (size_t)y * w * 4;
        for (int x = 0; x < w; x++) {
          unsigned char b = src[0], g = src[1], r = src[2], a = src[3];
          d[0] = (unsigned char)(((int)b * a) / 255);
          d[1] = (unsigned char)(((int)g * a) / 255);
          d[2] = (unsigned char)(((int)r * a) / 255);
          d[3] = a;
          src += 4;
          d += 4;
        }
...
HBITMAP TrayManagerPlugin::LoadIconBitmap(const std::string& base64) {
  std::vector<unsigned char> bytes = DecodeBase64(base64);
  if (bytes.empty()) return nullptr;
  return LoadPngBitmap(bytes);
}
...
std::optional<LRESULT> TrayManagerPlugin::HandleWindowProc(HWND hWnd,
  ...
  if (message == WM_DESTROY) {
    ...
    if (mis->CtlType == ODT_MENU) {
      auto it = menu_visuals_.find((int)mis->itemID);
      if (it != menu_visuals_.end()) {
        ...
        SIZE text = {};
        GetTextExtentPoint32W(hdc, it->second.label.c_str(),
                              (int)it->second.label.size(), &text);
        ...
        mis->itemHeight = (UINT)(max(22, text.cy + 6));
        mis->itemWidth = (UINT)(text.cx + iconW + 16);
      }
    }
    result = TRUE;
    ...
    if (dis->CtlType == ODT_MENU) {
      auto it = menu_visuals_.find((int)dis->itemID);
      if (it != menu_visuals_.end()) {
        HDC hdc = dis->hDC;
        RECT rc = dis->rcItem;
        ...
        if (it->second.bitmap) {
          ...
          HGDIOBJ old = SelectObject(mem, it->second.bitmap);
          BLENDFUNCTION bf = {AC_SRC_OVER, 0, 255, AC_SRC_ALPHA};
          AlphaBlend(hdc, x, y, bmp.bmWidth, bmp.bmHeight, mem, 0, 0,
                     bmp.bmWidth, bmp.bmHeight, bf);
        ...
        SetTextColor(hdc, grayed ? GetSysColor(COLOR_GRAYTEXT)
                                 : (selected ? GetSysColor(COLOR_HIGHLIGHTTEXT)
                                             : GetSysColor(COLOR_MENUTEXT)));
        RECT tr = {x, rc.top, rc.right - 6, rc.bottom};
        DrawTextW(hdc, it->second.label.c_str(), -1, &tr,
...
void TrayManagerPlugin::PopUpContextMenu(
  ...
  (void)bringAppToFront;  // kept for API compatibility; not needed on Win32
  ...
  // A tray popup must run from the foreground window, otherwise Windows does
  // not track it as the active menu: clicking anywhere outside would not
  // dismiss it. TPM_RIGHTBUTTON makes it behave like a context menu, and the
  // trailing WM_NULL lets the window re-process the click that closed it.
